...
export interface Category {
  ...
  label: string;
  group: CategoryGroup;
  /** Deezer playlist backing the category. */
  playlistId: string;
...
const YEAR_PLAYLIST_IDS: [year: number, playlistId: string][] = [
  [2023, '12345467671'],
  [2022, '12345460211'],
  [2021, '12345421311'],
  [2020, '3185085222'],
...
const YEARS: Category[] = YEAR_PLAYLIST_IDS.map(([year, playlistId]) => ({
  id: `year-${year}`,
  label: `Top Hits ${year}`,
  group: 'year' as const,
  playlistId,
 ...
 */
const GENRE_DEFS: [label: string, playlistId: string][] = [
...
const GENRES: Category[] = GENRE_DEFS.map(([label, playlistId]) => ({
